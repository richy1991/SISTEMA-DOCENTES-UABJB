    ...
    try {
      ...
      if (resp && typeof resp === 'object') {
        ...
        for (const k of Object.keys(resp)) {
          newFieldErrors[k] = Array.isArray(resp[k]) ? String(resp[k].join(' ')) : String(resp[k]);
        }
        setFieldErrors(newFieldErrors);
        toast.error('Error validando campos. Revisa el formulario.');
      } else {
        const msg = resp || err?.message || 'Error al guardar ítem';
        setError(String(msg));
