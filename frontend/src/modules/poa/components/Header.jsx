...
import logo from '../assets/LogoCIS.jpeg';
...
const Header = ({ 
  ...
  const sidebarOffset = sidebarExpanded ? 'left-72' : 'left-16';
  const headerWidth = sidebarExpanded ? 'calc(100% - 18rem)' : 'calc(100% - 4rem)';

  return (
    <header style={{ width: headerWidth }} className={`${themeConfig.headerBg} ${themeConfig.headerText} flex items-center px-6 py-4 fixed top-0 right-0 z-30 transform transition-all duration-300 ease-in-out ${sidebarOffset} ${showHeader ? `translate-y-0 opacity-100 ${themeConfig.headerShadow} ${themeConfig.headerBorder}` : '-translate-y-full opacity-0 pointer-events-none'}`}>
