    ...
    <Modal onClose={onClose}>
      <div className="modal-panel rounded-xl w-11/12 max-w-2xl">
        ...
        <div className="px-8 py-6 modal-body">
          ...
          {error && <div className="text-red-600 dark:text-red-400 mb-4">{JSON.stringify(error)}</div>}

          <form onSubmit={handleCreate} className="space-y-5">

            {/* Fila 1: Gestión + Fecha */}
            <div className="grid grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Gestión (Año)</label>
                <input
                  name="gestion"
                  value={form.gestion}
                  ...
                  readOnly={!!initialGestion}
                  disabled={!!initialGestion}
                />
                {initialGestion && <p className="text-xs text-gray-500 dark:text-slate-400 mt-1">Gestión seleccionada: {initialGestion}</p>}
              </div>
              <div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">Estado del documento</label>
              <div className="grid grid-cols-4 gap-3">
                {[
                  { value: 'elaboracion', label: 'En elaboración', color: 'bg-yellow-100 dark:bg-yellow-900/30 border-yellow-400 dark:border-yellow-700 text-yellow-800 dark:text-yellow-200' },
                  { value: 'revision',   label: 'En revisión',    color: 'bg-blue-100 dark:bg-blue-900/30 border-blue-400 dark:border-blue-700 text-blue-800 dark:text-blue-200' },
                  { value: 'aprobado',   label: 'Aprobado',       color: 'bg-green-100 dark:bg-green-900/30 border-green-400 dark:border-green-700 text-green-800 dark:text-green-200' },
                  { value: 'ejecucion',  label: 'En ejecución',   color: 'bg-purple-100 dark:bg-purple-900/30 border-purple-400 dark:border-purple-700 text-purple-800 dark:text-purple-200' },
                ].map(opt => (
                  <button
                    key={opt.value}
                    type="button"
                    onClick={() => setForm(f => ({ ...f, estado: opt.value }))}
                    className={`border-2 rounded-lg px-2 py-2 text-xs font-semibold transition-all ${form.estado === opt.value ? opt.color + ' ring-2 ring-offset-1 ring-current' : 'bg-gray-50 dark:bg-slate-800 border-gray-200 dark:border-slate-600 text-gray-500 dark:text-slate-400 hover:border-gray-400 dark:hover:border-slate-500'}`}
                  >
                    {opt.label}
                  </button>
                ))}
                    ...
                    {showElabDropdown && elabFilteredPersonas.length > 0 && (
                      <ul className="absolute z-50 mt-1 w-full max-w-full bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-600 rounded shadow-lg max-h-56 overflow-auto">
                        {elabFilteredPersonas.map((p, i) => (
                          <li key={p.id} onMouseEnter={() => setElabHighlight(i)} onMouseDown={ev => ev.preventDefault()} onClick={() => { setForm(f => ({ ...f, elaborado_por: p.id })); setElabQuery(p.nombre || p.user?.username || ''); setShowElabDropdown(false); }} className={`px-3 py-2 cursor-pointer ${elabHighlight === i ? 'bg-blue-100 dark:bg-slate-700' : 'hover:bg-gray-100 dark:hover:bg-slate-700'}`}>
                    ...
                    {showJefeDropdown && jefeFilteredPersonas.length > 0 && (
                      <ul className="absolute z-50 mt-1 w-full max-w-full bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-600 rounded shadow-lg max-h-56 overflow-auto">
                        {jefeFilteredPersonas.map((p, i) => (
                          <li key={p.id} onMouseEnter={() => setJefeHighlight(i)} onMouseDown={ev => ev.preventDefault()} onClick={() => { setForm(f => ({ ...f, jefe_unidad: p.id })); setJefeQuery(p.nombre || p.user?.username || ''); setShowJefeDropdown(false); }} className={`px-3 py-2 cursor-pointer ${jefeHighlight === i ? 'bg-blue-100 dark:bg-slate-700' : 'hover:bg-gray-100 dark:hover:bg-slate-700'}`}>
                            {p.nombre || p.user?.username}
                          </li>
                        ))}
                      </ul>
